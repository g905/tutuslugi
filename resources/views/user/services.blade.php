            ...
            <div class="page-col-left s-895">
                ...
                <div class="user-cabinet-section">
                    ...
                    <section class="user-text-default">
                        ...
                        <div class="user-profile-service-tab">
                            <button id="user-profile-show-services" class="active">Категории</button>
                            <button id="user-profile-show-prices" @if(empty($Services)||$Services->count()==0) class="d-none" @endif>Услуги и цены</button>
                        </div>


                        <div class="user-profile-service-section-tab">
                        <form method="POST" action="{{ route('user.service') }}/" name="form-user-services" class="form-user-services">
                            @csrf
                                ...
                                @if(!empty($Categories))
                                    @foreach($Categories as $category)
                                        ...
                                        <div class="regions-list-item">
                                            <div class="regions-list-item-name">{{$category->name}} <i><svg width="11" height="7" viewBox="0 0 11 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    ...
                                                    @foreach($category->children as $scategory)
                                                        <div class="regions-list-item-city">
                                                            <label class="checkbox-style">
                                                                <input @if(!empty($UserCategories[$scategory->id])) checked @endif name="category[{{$category->id}}][]" type="checkbox" value="{{$scategory->id}}">
                        ...
                        <div class="user-profile-prices-section-tab d-none">
                            @include('user.prices')
                        </div>
                    </section>
